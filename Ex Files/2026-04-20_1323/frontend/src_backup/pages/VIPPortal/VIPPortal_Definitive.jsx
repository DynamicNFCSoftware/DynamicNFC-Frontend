...
export default function VIPPortal() {
  ...
  // Track page load
  useEffect(() => { initSession({ cardId: 'VISTA001', visitorType: 'vip', visitorName: 'Khalid Al-Rashid', portalName: 'Vista VIP' }); trackEvent("portal_opened", { language: lang }); }, []);
  ...
  const submitBooking = () => {
    ...
    trackLegacy('book_viewing', { unitId: form.unit || 'general', name: form.name });
  ...
  return (
    <div className="vp" dir={t.dir}>
      ...
      <section className="vp-hero">
        ...
        <div className="vp-hero-ct">
          ...
          <div className="vp-hacts">
            <button className="vp-btn-g" onClick={() => { trackEvent("cta_explore"); trackLegacy('cta_explore', {}); resRef.current?.scrollIntoView({ behavior: "smooth" }); }}>{t.hero.cta} →</button>
            <button className="vp-btn-o" onClick={() => { trackEvent("cta_booking"); trackLegacy('cta_booking', {}); bookRef.current?.scrollIntoView({ behavior: "smooth" }); }}>{t.hero.ctaSecondary}</button>
          </div>
        </div>
      </section>
      ...
      <Link to="/enterprise/crmdemo/roi-calculator" className="vp-roi-banner" onClick={() => { trackEvent("roi_calculator_click"); trackLegacy('roi_calculator_click', {}); }}>
