...
export default function MarketplacePortal() {
  ...
  useEffect(() => { initSession({ cardId: 'VISTA003', visitorType: 'anonymous', visitorName: null, portalName: 'Vista Marketplace' }); trackEvent("marketplace_visit"); }, []);
  ...
  const requireLead = (action) => {
    ...
    trackLegacy('lead_form_shown', { trigger: 'high_intent_action' });
  };
  const handleLeadSubmit = (e) => {
    ...
    trackLegacy('lead_captured', { leadName: name, leadEmail: email });
  ...
  const toggleCompare = (unitId) => {
    setCompareList((prev) => {
      ...
      trackLegacy('comparison_view', { unitId });
  ...
  const openDetail = (unit) => { setSelectedUnit(unit); trackEvent("view_unit", { unitId: unit.id, unitName: unit.name.en }); trackLegacy('view_unit', { unitId: unit.id, unitName: unit.name.en }); };
  const openFloor = (unit) => requireLead(() => { setModalUnit(unit); setModal("floorplan"); trackEvent("view_floorplan", { unitId: unit.id }); trackLegacy('view_floorplan', { unitId: unit.id }); showToast(t.toast.floorPlan); });
  const openBrochure = (unit) => requireLead(() => { setModalUnit(unit); setModal("brochure"); trackEvent("download_brochure", { unitId: unit.id }); trackLegacy('download_brochure', { unitId: unit.id }); showToast(t.toast.brochure); });
  const openPayment = (unit) => requireLead(() => { setModalUnit(unit); setModal("payment"); setPayPlan("60/40"); trackEvent("explore_payment_plan", { unitId: unit.id }); trackLegacy('explore_payment_plan', { unitId: unit.id }); });
  const openCompare = () => { setModal("compare"); };
  const reqPricing = (unit) => requireLead(() => { trackEvent("request_pricing", { unitId: unit.id }); trackLegacy('request_pricing', { unitId: unit.id }); showToast(t.toast.pricing); });
  const handleBooking = (unit) => requireLead(() => { trackEvent("book_viewing", { unitId: unit?.id || "general" }); trackLegacy('book_viewing', { unitId: unit?.id || 'general' }); showToast(t.toast.booking); });
  ...
  return (
    <div className="mp" dir={t.dir}>
      ...
      <section className="mp-hero">
        ...
        <div className="mp-hero-ct">
          ...
          <div className="mp-hacts">
            <button className="mp-btn-d" onClick={() => { trackEvent("cta_browse"); trackLegacy('cta_browse', {}); resRef.current?.scrollIntoView({ behavior: "smooth" }); }}>{t.hero.cta} →</button>
        ...
        <div className="mp-sh mp-rv">
          ...
          <div className="mp-filters">
            {[{ key: "all", l: t.filters.all }, { key: "penthouse", l: t.filters.penthouse }, { key: "3bed", l: t.filters.bed3 }, { key: "2bed", l: t.filters.bed2 }].map(f => (
              <button key={f.key} className={`mp-ftab ${filter === f.key ? "on" : "off"}`} onClick={() => { setFilter(f.key); trackEvent("filter_units", { filter: f.key }); trackLegacy('filter_units', { filter: f.key }); }}>{f.l}</button>
            ))}
          </div>
        </div>
      ...
      <Link to="/enterprise/crmdemo/roi-calculator" className="mp-roi-banner" onClick={() => { trackEvent("roi_calculator_click"); trackLegacy('roi_calculator_click', {}); }}>
