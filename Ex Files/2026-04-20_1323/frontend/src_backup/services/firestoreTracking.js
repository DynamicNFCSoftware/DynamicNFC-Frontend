 ...
 */
export function calculateEngagementScore(events, config = null) {
  if (!events || events.length === 0) return { score: 0, label: 'NEW', color: '#9ca3af', stage: 'none' };
  ...
  if (score >= hotT || hasAction) {
    stage = 'action'; label = 'HOT LEAD'; color = '#dc2626';
  } else if (score >= warmT || hasIntent) {
    stage = 'intent'; label = 'WARM'; color = '#f59e0b';
  } else if (score >= intT || hasEngage) {
    stage = 'engage'; label = 'INTERESTED'; color = '#0176d3';
  } else {
    stage = 'browse'; label = 'NEW'; color = '#9ca3af';
  }
...
export function describeEvent(event) {
  ...
  if (!schema) return event.event;

  let desc = schema.label;
  const d = event.details || event.data || {};

  if (d.unitId) desc += ` \u2014 ${d.unitName || d.unitId}`;
  if (d.towerName) desc += ` \u2014 ${d.towerName}`;
  if (d.durationSeconds) desc += ` (${d.durationSeconds}s session)`;
