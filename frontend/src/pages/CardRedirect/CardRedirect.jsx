// PRIMARY redirect handler for /c/:cardId
// Reads smartcards/{cardId} from Firestore, logs tap to behaviors,
// then redirects to card.redirectUrl
// Cloud Function version in functions/index.js is deactivated.
...
export default function CardRedirect() {
  ...
  useEffect(() => {
    ...
    async function redirect() {
      try {
        // Rate limiting: max 1 tap per card per 5 seconds
        const rlKey = `dnfc_tap_${cardId}`;
        const lastTap = parseInt(sessionStorage.getItem(rlKey) || '0', 10);
        const now = Date.now();
        const rateLimited = now - lastTap < 5000;
        ...
        if (!rateLimited) {
          sessionStorage.setItem(rlKey, String(now));
        ...
        // Check scheduled redirects
        let targetUrl = card.redirectUrl;
        if (Array.isArray(card.scheduledRedirects) && card.scheduledRedirects.length > 0) {
          const now = Date.now();
          const active = card.scheduledRedirects.find(sr => {
            const start = sr.startDate?.toMillis?.() ?? sr.startDate;
            const end = sr.endDate?.toMillis?.() ?? sr.endDate;
            return start <= now && now <= end && sr.url;
          });
          if (active) targetUrl = active.url;
        }
  ...
  return (
    <div style={{
      ...
      <SEO title="Card Redirect" description="NFC card redirect with tap tracking." path="/c" />
      {error ? (
