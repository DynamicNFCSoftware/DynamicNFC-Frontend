...
// (single-source re-export). en/ar (reference scope).
// ═══════════════════════════════════════════════════════════════════

const OWNER = "Prince Nasser Al-Saud";
const VESSEL = "Azimut Grande 35 Metri";
const VESSEL_PRICE = "SAR 38,000,000";
const MARINA = "Marina Yachts VIP Berth, Dubai Marina";
const TO_EMAIL = "nasser.alsaud@gulfmarina.help";
...
const TR = {
  en: {
    ...
    footer: "AI Concierge Demo for", footerLink: "Dynamic NFC",
  },
  ar: {
    ...
    footer: "عرض الكونسيرج الذكي لـ", footerLink: "Dynamic NFC",
  },
};
...
const REAL_RESULTS = {
  ...
  gmail: {
    ...
    attachment: "Marina_Yachts_Azimut_Grande35_Sea_Trial.pdf",
...
const TERMINAL_LINES = {
  trigger: [
    ...
    { type: "data", text: `Interest: ${VESSEL} | Budget: SAR 35M\u201350M` },
  ...
  canva: [
    ...
    { type: "ok", text: "Specification generated \u2014 Marina_Yachts_Azimut_Grande35_Sea_Trial.pdf (3.1MB)" },
  ],
  gmail: [
    { type: "cmd", text: `mcp.gmail.create_draft({to: '${TO_EMAIL}'})` },
    ...
    { type: "data", text: "Attaching: Marina_Yachts_Azimut_Grande35_Sea_Trial.pdf" },
  ...
  docusign: [
    ...
    { type: "cmd", text: `mcp.docusign.send_envelope({recipient: '${TO_EMAIL}'})` },
...
export default function YachtAIDemo() {
  ...
  const trackEvent = useCallback(
    (event, data) => trackPortalEvent("vip", { id: `${regionId}-yacht-ai`, name: OWNER }, event, { portal: "yacht", unitName: VESSEL, unitType: "motor", tower: MARINA, ...data }),
    [regionId]
  );
  ...
  useEffect(() => { trackEvent("portal_opened", { scenario: "ai_sea_trial" }); }, []);
  ...
  const runStep = useCallback(async (stepIdx) => {
    ...
    if (cfg.key === "gmail" && googleToken) {
      ...
      const htmlBody = buildYachtEmailHtml({
        ownerName: OWNER, vesselName: VESSEL, vesselPrice: VESSEL_PRICE,
      ...
      gmailPromise = createGmailDraft(googleToken, { to: TO_EMAIL, subject: REAL_RESULTS.gmail.subject, htmlBody, senderName: "Marina Yachts" })
    ...
    <div className={`yai ${theme}`} dir={isRtl ? "rtl" : "ltr"}>
      ...
      <header className="yai-hd">
        ...
        <div className="yai-hd-right">
          ...
          <button className="yai-lang" onClick={() => setLang(lang === "en" ? "ar" : "en")}>
                      ...
                      <div className="yai-result-area">
                        {cfg.key === "trigger" && (
                          <div className="yai-profile">
                            <div className="yai-profile-avatar">NA</div>
                            <div className="yai-profile-info">
                              <h4>{OWNER} <span className="yai-live-tag">{t("detected")}</span></h4>
                              <p>Platinum VIP &bull; Interest: {VESSEL} &bull; Budget: SAR 35M–50M<br />Last activity: 3 days ago &bull; VIP ID: YV-001</p>
                            </div>
                            <div className="yai-profile-score"><div className="yai-profile-score-num">94</div><div className="yai-profile-score-label">{t("leadScore")}</div></div>
                          </div>
                        )}
            ...
            <div className="yai-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="yai-em-body">
                ...
                <div className="yai-em-content">
                  ...
                  <div className="yai-em-detail"><span>{VESSEL} — {VESSEL_PRICE}</span></div>
