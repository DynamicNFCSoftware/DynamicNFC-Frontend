...
function formatTime(ts, locale) {
  ...
  return ts.toDate().toLocaleTimeString(locale, {
    hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: locale.startsWith('en'),
  });
}
...
function FunnelBar({ stage, t }) {
  const stages = ['browse', 'engage', 'intent', 'action'];
  const labels = [t('awareness'), t('interest'), t('intent'), t('actionLabel')];
...
export default function AdminVIPProfile() {
  ...
  // Scoring
  const scoreResult = calculateEngagementScore(events, null, {
    hot: t('scoreHotLead'),
    warm: t('scoreWarmLead'),
    interested: t('scoreInterested'),
    new: t('scoreNew'),
  });
  ...
  // CTA Summary
  const ctaSummary = [
    { event: 'pricing_request', label: t('requestedPricing'), done: false, count: 0 },
    { event: 'book_viewing', label: t('bookedViewing'), done: false, count: 0 },
    { event: 'brochure_download', label: t('actionBrochure'), done: false, count: 0 },
    { event: 'floorplan_download', label: t('evtDownloadedFloorPlan'), done: false, count: 0 },
    { event: 'payment_plan', label: t('alertPaymentPlan'), done: false, count: 0 },
    { event: 'roi_calculator', label: t('roiShort'), done: false, count: 0 },
    { event: 'contact_advisor', label: t('alertAdvisorContact'), done: false, count: 0 },
  ];
  ...
  return (
    <div style={{ maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginTop: '0.25rem' }}>
            <span style={{ fontSize: '0.82rem', color: '#6b7280' }}>{t('cardLabel')}: {cardId}</span>
            ...
            <span style={{ fontSize: '0.82rem', color: '#6b7280' }}>{card?.totalTaps || 0} {t('tapsLabel')}</span>
          </div>
        </div>
        <button onClick={fetchData} style={{ padding: '0.4rem 1rem', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#374151', cursor: 'pointer', fontSize: '0.82rem', fontFamily: 'inherit' }}>{t('refresh')}</button>
      </div>

      {/* Engagement Score */}
      <div style={sectionStyle}>
        <div style={labelStyle}>{t('engagementScore')}</div>
        ...
        <FunnelBar stage={scoreResult.stage} t={t} />
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1rem' }}>
        {[
          { val: uniqueSessions, lbl: t('totalVisits'), color: '#0176d3' },
          { val: durationStr, lbl: t('totalTime'), color: '#16a34a' },
          { val: unitsViewed, lbl: t('unitsViewed'), color: '#f59e0b' },
          { val: ctaClicks, lbl: t('ctasClicked'), color: '#e63946' },
        ].map((s, i) => (
      ...
      {/* Session Breakdown */}
      <div style={sectionStyle}>
        <div style={labelStyle}>{t('sessionsCount', { count: sessions.length })}</div>
        {sessions.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '2rem', color: '#9ca3af' }}>{t('noSessionsYet')}</div>
        ) : (
            ...
            return (
              <div key={sid} style={{ borderBottom: '1px solid #f3f4f6' }}>
                ...
                >
                  ...
                  <span style={{ color: '#9ca3af', fontSize: '0.75rem', minWidth: 100 }}>{formatDate(sData.start, locale)} {formatTime(sData.start, locale)}</span>
                  <span style={{ color: getCategoryColor(highestCat), fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase' }}>
                    {highestCat === 'action' ? t('actionLabel') : highestCat === 'intent' ? t('intent') : highestCat === 'engage' ? t('interest') : t('awareness')}
                  </span>
                  <span style={{ color: '#6b7280', fontSize: '0.75rem' }}>{sData.events.length} {t('eventsText')}</span>
                  {sData.duration > 0 && <span style={{ color: '#6b7280', fontSize: '0.75rem' }}>{sData.duration}{t('secondsShort')}</span>}
                  ...
                  <div style={{ paddingLeft: '1.5rem', paddingBottom: '0.5rem' }}>
                    {sData.events.map(ev => (
                      <div key={ev.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', padding: '0.35rem 0', fontSize: '0.82rem' }}>
                        ...
                        <span style={{ color: '#9ca3af', fontSize: '0.72rem', minWidth: 90, flexShrink: 0 }}>{formatTime(ev.timestamp, locale)}</span>
                        <span style={{ color: '#374151', flex: 1 }}>{describeEvent(ev, t)}</span>
                        {ev.secondsInSession != null && <span style={{ color: '#d1d5db', fontSize: '0.7rem' }}>{ev.secondsInSession}{t('secondsShort')}</span>}
                      </div>
                    ))}
                  </div>
                )}
      ...
      {topUnits.length > 0 && (
        <div style={sectionStyle}>
          <div style={labelStyle}>{t('mostViewedUnits')}</div>
          {topUnits.map(([uid, info]) => (
            <div key={uid} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid #f3f4f6', fontSize: '0.85rem' }}>
              <div>
                ...
                <span style={{ color: '#6b7280', marginLeft: '0.5rem' }}>{info.count} {t('viewsText')}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* CTA Summary */}
      <div style={sectionStyle}>
        <div style={labelStyle}>{t('ctaSummary')}</div>
        {ctaSummary.map(cta => (
          <div key={cta.event} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', fontSize: '0.85rem' }}>
            ...
            <span style={{ color: cta.done ? '#374151' : '#9ca3af' }}>
              {cta.label}{cta.count > 1 ? ` (${cta.count}x)` : cta.done ? '' : ` - ${t('notYet')}`}
            </span>
          </div>
        ))}
      </div>
